...
namespace Andrei15193.Kepler.Language.Syntax
{
		...
		: RuleSet<Lexicon>
	{
		public KeplerRuleSet(ILanguage<Lexicon> language = null)
			: base(language ?? Language<Lexicon>.Default, ignoreRuleNameCase: true)
		{
			Add(Name,
				RuleNode<Lexicon>.Atom(Lexicon.Identifier));

			Add(ArithmeticConstant,
			...
			Add(FactParameter,
				...
				RuleNode<Lexicon>.Rule(ArithmeticConstant));
			...
			Add(Body,
				...
				RuleNode<Lexicon>.RuleSequence("statementSequence"),
				RuleNode<Lexicon>.Atom(Lexicon.End));

			Add("statementSequence",
				RuleNode<Lexicon>.Rule(Statement));
			...
			Add(BooleanOperand,
				RuleNode<Lexicon>.Rule(BooleanConstant));
			Add(BooleanOperand,
				RuleNode<Lexicon>.Rule(QualifiedIdentifier));
			Add(BooleanOperand,
				RuleNode<Lexicon>.Rule(FunctionCall));
		...
		public ProgramNode Parse(ScanResult<Lexicon> scanResult)
		{
			if (scanResult == null)
				throw new ArgumentNullException("scanResult");
			return new ProgramNode(Parse(scanResult.ScannedAtoms, Program));
		}

		public const string Name = "name";
		public const string ArithmeticConstant = "arithmeticConstant";
