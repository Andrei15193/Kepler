...
{
    public sealed class EndSymbol
        : TerminalSymbol
    {
        public EndSymbol(ScannedAtom<Lexicon> atom, ILanguage<Lexicon> language)
            : base(SymbolNodeType.End)
        {
            if (atom != null)
                if (language != null)
                    if (atom.Code == Lexicon.End)
                    {
                        _symbol = language.GetSymbol(atom.Code);
                        _atom = atom;
                    }
                    else
                        throw ExceptionFactory.CreateExpectedSymbol("end", atom.Line, atom.Column);
                else
