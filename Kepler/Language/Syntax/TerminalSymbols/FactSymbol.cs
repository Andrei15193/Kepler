...
{
    public sealed class FactSymbol
        : TerminalSymbol
    {
        public FactSymbol(ScannedAtom<Lexicon> atom, ILanguage<Lexicon> language)
            : base(SymbolNodeType.Fact)
        {
            if (atom != null)
                if (language != null)
                    if (atom.Code == Lexicon.Fact)
                    {
                        _symbol = language.GetSymbol(atom.Code);
                        _atom = atom;
                    }
                    else
                        throw ExceptionFactory.CreateExpectedSymbol("fact", atom.Line, atom.Column);
                else
