...
{
    public sealed class OpeningAngleBracketSymbol
         : TerminalSymbol
    {
        public OpeningAngleBracketSymbol(ScannedAtom<Lexicon> atom, ILanguage<Lexicon> language)
            : base(SymbolNodeType.OpeningAngleBracket)
        {
            if (atom != null)
                if (language != null)
                    if (atom.Code == Lexicon.LessThan)
                    {
