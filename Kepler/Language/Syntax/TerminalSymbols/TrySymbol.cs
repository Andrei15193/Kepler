...
{
    public sealed class TrySymbol
          : TerminalSymbol
    {
        public TrySymbol(ScannedAtom<Lexicon> atom, ILanguage<Lexicon> language)
            : base(SymbolNodeType.Try)
        {
            if (atom != null)
                if (language != null)
                    if (atom.Code == Lexicon.Try)
                    {
                        _symbol = language.GetSymbol(atom.Code);
                        _atom = atom;
                    }
                    else
                        throw ExceptionFactory.CreateExpectedSymbol("try", atom.Line, atom.Column);
                else
