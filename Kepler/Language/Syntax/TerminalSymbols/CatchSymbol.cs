...
{
    public sealed class CatchSymbol
          : TerminalSymbol
    {
        public CatchSymbol(ScannedAtom<Lexicon> atom, ILanguage<Lexicon> language)
            : base(SymbolNodeType.Catch)
        {
            if (atom != null)
                if (language != null)
                    if (atom.Code == Lexicon.Catch)
                    {
                        _symbol = language.GetSymbol(atom.Code);
                        _atom = atom;
                    }
                    else
                        throw ExceptionFactory.CreateExpectedSymbol("catch", atom.Line, atom.Column);
                else
