...
{
    public sealed class StopSymbol
        : TerminalSymbol
    {
        public StopSymbol(ScannedAtom<Lexicon> atom, ILanguage<Lexicon> language)
            : base(SymbolNodeType.Stop)
        {
            if (atom != null)
                if (language != null)
                    if (atom.Code == Lexicon.Stop)
                    {
                        _symbol = language.GetSymbol(atom.Code);
                        _atom = atom;
                    }
                    else
                        throw ExceptionFactory.CreateExpectedSymbol("stop", atom.Line, atom.Column);
                else
